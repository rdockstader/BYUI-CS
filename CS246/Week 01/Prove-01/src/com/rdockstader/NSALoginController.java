...
public final class NSALoginController {
	...
	*/
	public static void hashUserPassword(User user) throws WeakPasswordException, Exception {

		if(user.getPassword().length() < 8) {
			throw new WeakPasswordException("Password length must be at least 8 characters.");
		}
		boolean hasInt = false;
		for(char e: user.getPassword().toCharArray()) {
		...
		if(!hasInt) {
			throw new WeakPasswordException("Password must contain an integer!");
		}
